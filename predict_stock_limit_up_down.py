...
def prepare_stock_features(stock_code, start_date, end_date):
    """获取指定股票的历史数据并生成特征"""
    df_stock = pro.daily(ts_code=stock_code, start_date=start_date, end_date=end_date)
    df_stock['trade_date'] = pd.to_datetime(df_stock['trade_date']).dt.strftime('%Y%m%d')
    df_stock = df_stock.set_index('trade_date')
    df_stock['return'] = df_stock['close'].pct_change()
    df_stock['target'] = (df_stock['return'].shift(-1) > 0).astype(int)
    return df_stock.dropna()

def train_stock_model(df, feature_cols, stock_code):
    X = df[feature_cols]
    y = df['target']
    ...
    print(f"{stock_code} 训练集准确率: {train_score:.4f}")
    print(f"{stock_code} 测试集准确率: {test_score:.4f}")
...
def main():
    ...
    # 准备市场特征
    df_market, feature_cols = prepare_features(df_index, daily_stats)
    ...
    for stock_code in stock_list:
        df_stock = prepare_stock_features(stock_code, start_date, end_date)
        df_combined = df_market.join(df_stock[['return', 'target']], how='inner')
        model, test_score = train_stock_model(df_combined, feature_cols, stock_code)
        
        # 过滤掉测试集准确率低于60%的模型
        if test_score < 0.6:
            report += f"{stock_code} 测试集准确率低于60%，不进行预测\n"
            continue
        
        # 预测未来两天的走势
        latest_data = df_combined[feature_cols].iloc[-1:]
        latest_date = df_combined.index[-1]
